...
class FtCtrl extends RemoteJobsControl
{
...
class FtClient
{
    ...
    private RemoteJobsUtility $utility;

    public function __construct(RemoteJobsUtility $utility, array $options)
    {
    ...
    public function get_utility(): RemoteJobsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RemoteJobsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RemoteJobsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RemoteJobsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RemoteJobsConfig::make_feature($name);
    ...
    private static function buildUrl(RemoteJobsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RemoteJobsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RemoteJobsResponse($fetched);
        }
    ...
    private function fail(RemoteJobsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RemoteJobsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
