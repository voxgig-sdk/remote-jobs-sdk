...
class RemoteJobsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RemoteJobsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RemoteJobsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RemoteJobsTestRunner.getenv("REMOTE_JOBS_TEST_LIVE")
        override = RemoteJobsTestRunner.getenv("REMOTE_JOBS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RemoteJobsTestRunner.getenv(key)
        ...
        explain = RemoteJobsTestRunner.getenv("REMOTE_JOBS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RemoteJobsTestRunner._test_control is not None:
            return RemoteJobsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RemoteJobsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RemoteJobsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RemoteJobsTestRunner.load_test_control()
...
def load_env_local():
    RemoteJobsTestRunner.load_env_local()


def env_override(m):
    return RemoteJobsTestRunner.env_override(m)


def entity_data(v):
    return RemoteJobsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RemoteJobsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RemoteJobsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RemoteJobsTestRunner.load_test_control()


def live_client_options():
    return RemoteJobsTestRunner.live_client_options()


def live_delay_ms():
    return RemoteJobsTestRunner.live_delay_ms()
