...
[MCP](https://modelcontextprotocol.io) server exposing the RemoteJobs SDK as
two agent tools — `remote-jobs_list` and `remote-jobs_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/remote-jobs-mcp)
make build

# 2. Provide credentials via the environment
export REMOTE_JOBS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user remote-jobs \
  -- /absolute/path/to/remote-jobs-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./remote-jobs-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// remote-jobs_list: first page of records
{ "entity": "job" }
{ "entity": "job", "query": { } }

// remote-jobs_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/remote-jobs-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user remote-jobs \
     -- "$PWD"/dist/*/remote-jobs-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `remote-jobs_list` and `remote-jobs_load` tools now appear
   in new sessions. Ask the agent to *"list job using remote-jobs"*
   and it calls `remote-jobs_list` with `{"entity":"job"}`.
...
```sh
export REMOTE_JOBS_APIKEY=sk_live_xxx            # API key
export REMOTE_JOBS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `remote-jobs_list` tool
...
### Call the `remote-jobs_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `remote-jobs_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `remote-jobs_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REMOTE_JOBS_APIKEY` | API key sent with every request. |
| `REMOTE_JOBS_BASE` | Optional override of the API base URL. |
...
```sh
./remote-jobs-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"remote-jobs_load","arguments":{"entity":"job","query":{"id":1}}}}'
```
