...
class RemoteJobsNetsimFeature extends RemoteJobsBaseFeature
{
    ...
    private function _simulate(RemoteJobsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(RemoteJobsContext $ctx, array $applied): void
    {
        $client = $this->client;
