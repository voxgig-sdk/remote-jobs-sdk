...
class RemoteJobsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = RemoteJobsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RemoteJobsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RemoteJobsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, RemoteJobsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return RemoteJobsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = RemoteJobsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = RemoteJobsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = RemoteJobsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RemoteJobsError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = RemoteJobsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RemoteJobsError("graphql_error",
                "RemoteJobsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
