...
class JobEntity
{
    ...
    {
        if ($args) {
            $this->_data = RemoteJobsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = RemoteJobsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws RemoteJobsError on failure (item-5 convention).
     */
