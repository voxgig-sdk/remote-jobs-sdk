...
{
    private static function utility(): RemoteJobsUtility
    {
        return new RemoteJobsUtility();
    }
    ...
    private static function ctx(array $over = []): RemoteJobsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RemoteJobsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RemoteJobsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RemoteJobsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RemoteJobsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RemoteJobsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RemoteJobsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RemoteJobsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RemoteJobsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RemoteJobsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RemoteJobsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RemoteJobsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RemoteJobsResponse([]),
            'result' => new RemoteJobsResult([]),
        ]);
        [, $err] = RemoteJobsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RemoteJobsSpec([]),
            'response' => null,
            'result' => new RemoteJobsResult([]),
        ]);
        [, $err] = RemoteJobsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RemoteJobsSpec(['step' => 's']),
            'response' => new RemoteJobsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RemoteJobsResult([]),
        ]);
        [, $err] = RemoteJobsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RemoteJobsSpec(['step' => 's']),
            'response' => new RemoteJobsResponse(self::resp(200, ['v' => 1])),
            'result' => new RemoteJobsResult([]),
        ]);
        [, $err] = RemoteJobsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RemoteJobsSpec(['step' => 's']),
            'response' => new RemoteJobsResponse(self::resp(200, ['v' => 2])),
            'result' => new RemoteJobsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RemoteJobsMakeResponse::call($ctx);
    ...
    {
        $preset = new RemoteJobsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RemoteJobsSpec([]),
            'response' => new RemoteJobsResponse([]),
            'result' => new RemoteJobsResult([]),
        ...
        [$got, $err] = RemoteJobsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RemoteJobsResult([])]);
        [, $err] = RemoteJobsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RemoteJobsSpec([]), 'result' => null]);
        [, $err] = RemoteJobsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RemoteJobsSpec(['step' => 's']),
            'result' => new RemoteJobsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RemoteJobsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RemoteJobsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RemoteJobsSpec(['step' => 's']),
            'result' => new RemoteJobsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RemoteJobsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RemoteJobsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RemoteJobsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RemoteJobsSpec([]),
            'result' => new RemoteJobsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RemoteJobsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RemoteJobsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RemoteJobsError('boom', 'boom');
        $utility->fetcher = function (RemoteJobsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RemoteJobsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RemoteJobsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RemoteJobsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RemoteJobsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RemoteJobsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RemoteJobsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RemoteJobsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RemoteJobsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RemoteJobsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RemoteJobsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RemoteJobsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RemoteJobsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RemoteJobsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RemoteJobsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RemoteJobsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RemoteJobsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RemoteJobsResult(['ok' => false])]);
        $this->expectException(RemoteJobsError::class);
        RemoteJobsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RemoteJobsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RemoteJobsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RemoteJobsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RemoteJobsBaseFeature();
        $b = new RemoteJobsBaseFeature();
        RemoteJobsFeatureAdd::call($ctx, $a);
        RemoteJobsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RemoteJobsBaseFeature
    {
        $f = new RemoteJobsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RemoteJobsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RemoteJobsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RemoteJobsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RemoteJobsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RemoteJobsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RemoteJobsResponse(['headers' => null]),
            'result' => new RemoteJobsResult([]),
        ]);
        RemoteJobsResultHeaders::call($ctx);
