...
import (
	...
	omni "github.com/voxgig-sdk/remote-jobs-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RemoteJobsError); is {
				out := map[string]any{
					"name":    "RemoteJobsError",
...
func omniCtx(arg any, client *sdk.RemoteJobsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RemoteJobsSDK); is {
			client = live
