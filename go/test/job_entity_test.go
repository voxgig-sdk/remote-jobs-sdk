...
func TestJobEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set REMOTEJOBS_TEST_JOB_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func jobBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REMOTEJOBS_TEST_JOB_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"REMOTEJOBS_TEST_JOB_ENTID": idmap,
		"REMOTEJOBS_TEST_LIVE":      "FALSE",
		"REMOTEJOBS_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["REMOTEJOBS_TEST_JOB_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["REMOTEJOBS_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
			},
			extra,
		})
		client = sdk.NewRemoteJobsSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["REMOTEJOBS_TEST_EXPLAIN"] == "TRUE",
		live:          live,
