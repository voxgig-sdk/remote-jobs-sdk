...
function isRunnable(code) {
    return (/new\s+RemoteJobsSDK\b/.test(code) ||
        /\bRemoteJobsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+RemoteJobsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/RemoteJobsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
