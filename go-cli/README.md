...
boru-driven command-line client **and** interactive REPL for the RemoteJobs
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/remote-jobs-cli)
make build

# 2. See usage (words, entities, env vars)
./remote-jobs-cli --help

# 3. Provide credentials once, via the environment
export REMOTE_JOBS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./remote-jobs-cli list job

# 5. Override the API base URL for a single call
REMOTE_JOBS_BASE=https://api.example.com ./remote-jobs-cli list job

# 6. No arguments -> interactive REPL
./remote-jobs-cli
remote-jobs> list job
remote-jobs> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/remote-jobs-cli
   ```
...
Run with no arguments to open a REPL (prompt `remote-jobs>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/remote-jobs-cli`. |
...
(`./remote-jobs-cli <expr>`) and interactively (the REPL), and expressions compose the
