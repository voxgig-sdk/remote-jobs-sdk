// Command RemoteJobs-cli is a CLI + REPL for the RemoteJobs SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REMOTE_JOBS_APIKEY (api key), REMOTE_JOBS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REMOTE_JOBS_APIKEY carries the API key and
	// REMOTE_JOBS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REMOTE_JOBS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REMOTE_JOBS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRemoteJobsSDK(opts)
