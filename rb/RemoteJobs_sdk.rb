...
require_relative 'RemoteJobs_types'
...
class RemoteJobsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RemoteJobsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RemoteJobsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RemoteJobsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RemoteJobsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RemoteJobsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RemoteJobsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RemoteJobsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RemoteJobsError.new(
        "#{op}_allow",
        "RemoteJobsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RemoteJobsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RemoteJobsError.new(
        "graphql_error", "RemoteJobsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
