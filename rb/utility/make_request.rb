...
module RemoteJobsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RemoteJobsResponse.new({})
    ...
    if fetch_err
      ...
      response = RemoteJobsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RemoteJobsResponse.new(fetched)
    else
