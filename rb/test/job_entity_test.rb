...
class JobEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = RemoteJobsSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REMOTEJOBS_TEST_JOB_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def job_basic_setup(extra)
  ...
  entid_env_raw = ENV["REMOTEJOBS_TEST_JOB_ENTID"]
  ...
  if env["REMOTEJOBS_TEST_LIVE"] == "TRUE"
    ...
    client = RemoteJobsSDK.new(Helpers.to_map(merged_opts))
  end
