...
class JobEntity
  ...
  def data_set(args)
    if args
      @_data = RemoteJobsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = RemoteJobsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Job>, Array] the matching Job items; raises RemoteJobsError on failure
